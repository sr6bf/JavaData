...
public class SimBase {
    ...
    public int execute(byte instruction, byte oldPC) {
    	...
    	int_reserved = get_bits(instruction, idx1:7,idx2:8);
    	int icode = get_bits(instruction, idx1:4,idx2:7);
        int a = get_bits(instruction,idx1:2,idx2:4);
        int b = get_bits(instruction,idx1:0,idx2:2;)
        
        //select operation according to TOY ISA table
        if(reserved == 1) {
        	return oldPC;
        }
        if(icode == 0) {
        ...
        if(icode == 6) {
        	if(b == 0) {
        		R[a] = M[oldPC+1] //rA = read from memory at pc+1
        	}
        	if(b == 1) {
        		R[a] += M[oldPC + 1]; //rA += read from memory
        	}
        	if(b == 2) {
        		R[a] &= M[oldPC + 1];
        	}
      ...
      //maybe write a loop to check each one
